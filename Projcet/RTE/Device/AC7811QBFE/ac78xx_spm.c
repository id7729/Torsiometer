...
*/
int32_t SPM_EnableXOSC(uint8_t enable)
{
...
*/
int32_t SPM_EnablePLL(uint8_t enable)
{
    __IO uint32_t status = 0;
    uint32_t timeout = SPM_TIMEOUT_VALUE;
    int32_t ret = 0;
...
void NMI_Handler(void)
{
    ...
    {
        WRITEMEM32(REG_SPM_WAKEUP_IRQ, SPM_WAKEUP_IRQ_STATUS_NMI_Msk);  /* write 1 clear NMI IRQ flag */
    }

    /* check if xosc loss triggle nmi interrupt */
    if (READMEM32(REG_RESET_STATUS) & CKGEN_RESET_STATUS_XOSC_LOSS_STATUS_Msk)
    {
	/* close xosc,prevent continuous enter nmi handler */
	SPM_EnableXOSC(0);
        /* clear all reset status to prevent always run in nmi_handler */
        WRITEMEM32(REG_RESET_STATUS, CKGEN_RESET_STATUS_CLR_RESET_STATUS_Msk);  /* clear all reset status */
